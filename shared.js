...
const sharedRenderers = {
  ...
  "revise-topic": (c, v) => {
    ...
    return `<div class="component-preview sx-screen">
      ...
      ${row("▤", "Lesson 2", "NEW")}
      ${proposed
        ? `<div class="sx-lesson-row sx-rt"><i class="sx-lesson-icon sx-ex-icon">✎</i><span><strong class="sx-new">Review Entire ${unit}</strong><small>NEW · exercises</small></span><b>▶</b></div>`
        : `<div class="sx-lesson-row sx-rt"><i class="sx-lesson-icon sx-book-icon sx-bad-box">📖</i><span><strong class="sx-bad">Revise Whole Topic</strong><small>NEW</small></span><b>▶</b></div>`}
    </div>`;
  },
...
function sharedItemsFor(key) {
  ...
  const items = [
    ...
    { kind: "revise-topic", courses: ["cpa", "cfa", "lsat", "bar", "mcat", "nclex"], area: "Lessons · Topic list", title: "Revise Whole Topic row", changes: [["Revise Whole Topic", `Review Entire ${key === "cfa" ? "Module" : "Topic"}`], ["Book icon", "Exercise icon (it opens exercises, not a lesson)"]], rationale: `Verified in components/lessons/myPath/ReviseTopicButton.tsx. The last row of every ${key === "cfa" ? "Learning Module" : "topic"} starts exercises for the whole ${key === "cfa" ? "module" : "topic"}, but the book icon makes it look like another lesson. “Revise” is British; US learners say “review”.${key === "cfa" ? " On CFA this level is a Learning Module, so the label says Module." : ""} Agreed in the team thread and tracked in ONC2-7198 (Mokshit).` },
