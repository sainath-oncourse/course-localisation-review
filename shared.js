...
const sharedCourses = {
  cpa: {
    ...
    examples: {
      ...
      quiz: ["Simulations", "FAR practice", "Rapid review"],
      lessons: ["Leases", "Deferred taxes", "Review topic"],
    ...
    reminders: ["Review flashcards daily", "Simulated exam this Sunday", "Practice MCQs and TBS", "Read FAR notes"],
  ...
  cfa: {
    ...
    examples: {
      ...
      quiz: ["Item sets", "Ethics cases", "Rapid review"],
      lessons: ["Fixed income", "Financial statements", "Review topic"],
    ...
    reminders: ["Review flashcards daily", "Mock exam this Sunday", "Practice MCQs", "Read Ethics notes"],
  ...
  lsat: {
    ...
    examples: {
      ...
      quiz: ["Logical Reasoning", "Reading Comprehension", "Rapid review"],
      lessons: ["Flaw questions", "Assumption questions", "Review topic"],
      mnemonics: ["Conditional logic", "Reasoning flaws", "Passage structure"],
      weak: ["Recent drills", "Question-type gaps", "Study plan"],
    },
    reminders: ["Review flashcards daily", "Practice test this Sunday", "Do a Logical Reasoning drill", "Review Reading Comprehension notes"],
  ...
  bar: {
    ...
    examples: {
      ...
      lessons: ["Evidence", "Civil procedure", "Review topic"],
      mnemonics: ["Negligence", "Adverse possession", "Hearsay exceptions"],
      weak: ["Recent drills", "Subject gaps", "Study plan"],
    },
    reminders: ["Review flashcards daily", "Exam simulation this Sunday", "Practice multiple-choice questions", "Read Evidence outline"],
  ...
  mcat: {
    ...
    examples: {
      ...
      lessons: ["Enzyme kinetics", "Social psychology", "Review topic"],
    ...
    reminders: ["Review flashcards daily", "Full-length exam this Sunday", "Do a CARS passage daily", "Review Biochemistry notes"],
...
function sharedTools(c, proposed) {
  ...
  return [
    ...
    { icon: "≡", title: proposed && c.lessonsTool ? c.lessonsTool : "Find high-yield lessons", subtitle: "Open exam-focused lessons", key: "lessons" },
...
const sharedRenderers = {
  ...
  reminders: (c, v) => {
    ...
    return `<div class="component-preview rezzy-reminders-preview"><div class="rezzy-gallery-header"><span>‹</span><strong>Reminders</strong><span></span></div><div class="reminder-bell">♢</div><h3>Set your first reminder</h3><p>Ask ${c.tutor} to remind you to ${v === "proposed" ? "study, review, or practice" : "study, revise, or practise"} — whenever you need it.</p><button type="button" tabindex="-1">SET A REMINDER</button><small>Try one of these</small><div class="reminder-chips">${chips.map((x) => `<span>${x}</span>`).join("")}</div></div>`;
  },
  ...
  "chat-history-empty": (c, v) => {
    ...
    return `<div class="component-preview sx-screen">
      ...
      <div class="sx-empty"><strong class="${proposed ? "" : "sx-bad"}">${proposed ? `No ${c.tutor} conversations yet.` : "No rezzy conversations yet."}</strong><small>${proposed ? "Quiz Analysis tab: “No quiz analysis conversations yet.”" : "Quiz Analysis tab: “No quiz_analysis conversations yet.”"}</small></div>
    </div>`;
  },
  ...
  "revise-topic": (c, v) => {
    const proposed = v === "proposed";
    const unit = c.name === "CFA" ? "Module" : "Topic";
    const row = (icon, title, sub, cls) => `<div class="sx-lesson-row ${cls || ""}"><i class="sx-lesson-icon">${icon}</i><span><strong>${title}</strong><small>${sub}</small></span><b>${cls === "sx-rt" ? "▶" : "✓"}</b></div>`;
    return `<div class="component-preview sx-screen">
      <div class="sx-card"><small>${unit} 1 · 3 Lessons</small><strong>${c.examples.lessons[0]}</strong></div>
      ${row("▤", "Lesson 1", "Read · To Do")}
      ${row("▤", "Lesson 2", "NEW")}
      ${proposed
        ? `<div class="sx-lesson-row sx-rt"><i class="sx-lesson-icon sx-book-icon">📖</i><span><strong class="sx-new">Review Entire ${unit}</strong><small>NEW</small></span><b>▶</b></div>`
        : `<div class="sx-lesson-row sx-rt"><i class="sx-lesson-icon sx-book-icon">📖</i><span><strong class="sx-bad">Revise Whole Topic</strong><small>NEW</small></span><b>▶</b></div>`}
    </div>`;
  },
  ...
  "exercise-done": (c, v) => {
    const proposed = v === "proposed";
    const unit = c.name === "CFA" ? "Module" : "Topic";
    return `<div class="component-preview sx-screen sx-center-col">
      <h3 class="sx-title">${unit} Complete!</h3>
      <p class="sx-sub">You've finished all lessons in this ${unit.toLowerCase()}</p>
      <div class="sx-card"><strong>Accuracy</strong><small>82%</small></div>
      <button class="sx-cta" type="button" tabindex="-1"><span class="${proposed ? "" : "sx-bad"}" style="${proposed ? "" : "color:#ffd2cc"}">${proposed ? `Review ${unit}` : "Revise Topic"}</span></button>
    </div>`;
  },
  ...
  "video-empty": (c, v) => {
    const proposed = v === "proposed";
    return `<div class="component-preview sx-screen">
      <div class="sx-pills"><span>Deep Dive</span><span class="on">Rapid Review</span></div>
      <div class="sx-empty"><strong class="${proposed ? "sx-new" : "sx-bad"}">${proposed ? "No review videos yet for this subject" : "No revision videos yet for this subject"}</strong></div>
    </div>`;
  },
...
function sharedItemsFor(key) {
  ...
  const items = [
    ...
    { kind: "revise-topic", courses: ["cpa", "cfa", "lsat", "bar", "mcat", "nclex"], area: "Lessons · Topic list", title: "Revise Whole Topic row", changes: [["Revise Whole Topic", `Review Entire ${key === "cfa" ? "Module" : "Topic"}`]], rationale: `Verified in components/lessons/myPath/ReviseTopicButton.tsx. The last row of every ${key === "cfa" ? "Learning Module" : "topic"} reviews the whole ${key === "cfa" ? "module" : "topic"}. “Revise” is British; US learners say “review”, and every competitor for this course uses it. The label stays generic (no topic name) because you are already inside that topic.${key === "cfa" ? " On CFA this level is a Learning Module, so the label says Module." : ""} Agreed in the team thread; tracked in ONC2-7198.` },
    { kind: "exercise-done", courses: ["cpa", "cfa", "lsat", "bar", "mcat", "nclex"], area: "Lessons · Exercises", title: "Exercise completion button", changes: [["Revise Topic", `Review ${key === "cfa" ? "Module" : "Topic"}`]], rationale: `Verified in components/exercises/ContinueLearning.tsx:334. Shown when every lesson in the ${key === "cfa" ? "module" : "topic"} is done. Matches the “Review Entire ${key === "cfa" ? "Module" : "Topic"}” row; “revise” is British.` },
    { kind: "video-empty", courses: ["cpa", "cfa", "lsat", "bar", "mcat", "nclex"], area: "Lessons · Lesson page", title: "Rapid Review video empty state", changes: [["No revision videos yet for this subject", "No review videos yet for this subject"]], rationale: "Verified in components/videoFlix/VideoFlixBrowseContent.tsx:583. The tab is already called Rapid Review, but the empty message inserts the internal mode name “revision”." },
    ...
    { kind: "flashcard-loading", courses: ["lsat", "mcat", "nclex"], area: "Flashcards · Generate with AI", title: "AI generation loading state", changes: [["flashcards.search_loading_text", `Creating your ${c.name} flashcards…`]], rationale: "Verified in app/(app)/snippets/flashcards/create-prompt.tsx:282. The key is missing from the base copy file, so every non-medical course sees the raw key." },
    ...
    { kind: "emoji-picker", courses: ["cpa", "cfa", "lsat", "bar", "mcat"], area: "Flashcards · Save to deck", title: "New deck emoji picker", changes: [["Medical & Clinical Context category", c.name === "LSAT" || c.name === "BAR" ? "Law & Reasoning" : c.name === "MCAT" ? "Science & Biology" : "Finance & Accounting"], ["Preset “Recently used”: 🫁🚑😷💊🩺", "Study emojis, or the user's real recent picks"], ["Time, Practice & Revision (category)", "Time, Practice & Review"]], rationale: "Verified in components/flashcards/SearchSection/SaveToDeckBottomSheet.tsx:25-31. “Recently used” is a hard-coded medical preset, not the user's history, and one category is medical." },
    ...
    { kind: "membership-perks", courses: ["cpa", "cfa", "lsat", "bar", "mcat", "nclex"], area: "Membership", title: "Your Max perks", changes: [["100k+ question bank – PYQs, mock tests…", c.perksBank], ["40k+ flashcards", "Flashcards"], ["Bloom's mastery, projected rank", "Exam readiness, mastery"], ...(c.gamesNow ? [[c.gamesNow, c.games]] : [])], rationale: `Verified in components/pricing/Membership/index.tsx:36-67 (web too). PYQs, the medical bank sizes and “projected rank” (a NEET-PG idea) do not apply to ${c.name}.` },
    { kind: "cancel-flow", courses: ["cpa", "cfa", "lsat", "bar", "mcat", "nclex"], area: "Membership · Cancel flow", title: "Cancel flow features and discount", changes: [["Mini-games (Synapses, Clinical Rounds)", `Mini-games (${c.games ? c.games.replace(" & ", ", ") : "Synapses, Probe"})`], ["100k+ question bank – PYQs, mock tests…", c.discountBank]], rationale: `Verified in app/(app)/cancel-flow/index.tsx:158-176 and 614-702. Clinical Rounds is not available to ${c.name}; PYQs and medical bank sizes are wrong. Separately, the discount button offers Medical PG plans (engineering issue, not copy).` },
  ];
  return items
...
function whereFor(courseKey, kind) {
  ...
  const paths = {
    ...
    "revise-topic": "Lessons tab → open a subject → last row of any topic (after its lessons)",
    "exercise-done": "Lessons tab → open a lesson → Exercises → finish the last one when every lesson in the topic is read",
    "video-empty": "Lessons tab → open a lesson → video (film) icon → Rapid Review, for a subject with no videos",
